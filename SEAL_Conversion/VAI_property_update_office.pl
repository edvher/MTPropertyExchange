...
#
# Location: <Drive>\SEAL\PLOSSYS\server\dpf\scripts\vai
#
# Revision 2.0  2012/08/10 08:34:12  Rafetseder Harald I MT IT PS TC
# recoding and cleanup to use new PropertyExchange Tools (ver. 20120810 upwards)
# tools can now handle word/excel 2003/2010 file types with unicode support for the 2010 versions
#
...
my $Dpf4cPropertyUpdateProvideFiles = $Par{"DPF4C_PROPERTYUPDATE_PROVIDE_FILES"};
my $Dpf4cPropertyUpdateFile = $Par{"DPF4C_PROPERTYUPDATE_FILE"};
my $DirMaintecOffice = $ENV{"PLSTOOLS"} . "\\vai_tools\\MTPropertyExchange";
my $PropertyUpdateExe = "$DirMaintecOffice\\MT_PropertyExchange.exe";
my $VaiPropertyUpdateOfficeWrite = $Par{"VAI_PROPERTY_UPDATE_OFFICE_WRITE"};
my $Dpf4cCommonDestFomat = $Par{"DPF4C_COMMON_DEST_FORMAT"};
my $Dpf4cCommonTimeout = &libconvdpf2::GetDpf4CTimeout($Par{"DPF4C_COMMON_TIMEOUT"});
my $ErrorText;
my $PropertyUpdateLog = &ConcatPath ($JobDir, "PropertyUpdate.log");
...
my @cmd = ("$PropertyUpdateExe",   
		"-mode=import", # means to use WriteProp
		"-file=\"$Currfilename\"",
		"-txt=\"$Dpf4cPropertyUpdateFile\"",
		"-encoding=\"iso-8859-1\"");
&Log ("cmd=<@cmd>", "I");
...
if ($stat ne 0) {
	if ($CheckErrorText ne "") { $ErrorText .= $CheckErrorText . " MT informations:"; }
	$ErrorText .= &GetPropertyExchangeErrorText ($stat);
...
if ($VaiPropertyUpdateOfficeWrite eq "Y") {
    &Log ("Doing update on paper becaue of dpf parameter".
          ...
          "I");
    my @cmd = ("$PropertyUpdateExe",
            "-mode=macro", # means to use runmacro
            "-file=\"$Currfilename\"",
            "-macro=DLLSAP2Document -loglevel=DEBUG -clearlog -logfile=\"C:\\Temp\\PropExLog.log\""); 

    &Log ("cmd=<@cmd>", "I");
    $stat = &libconvdpf2::mysystem_array (\@cmd, 
                                     ("-timeout", "$Dpf4cCommonTimeout"));
	&libconvdpf2::__Append2LogFile ("C:\\Temp\\PropExLog.log");
    &Log ("status cmd=<$stat>", "I");
    if ($stat ne 0)
        {
        $ErrorText = &GetPropertyExchangeErrorText ($stat);
...
sub CheckValidFormat
    ...
    if (!(
             ...
             ($Currfiletype eq "xlt")))
        {
        &Log ("Info: Property update tool does ".
...
sub DoOfficeRegistry {
    ...
    my ($Dll, @Dlls);
    push (@Dlls, $ENV{"PLSTOOLS"} . "/vai_tools/MTPropertyExchange/dsofile.dll");
...
# -------------------------------------------------------
# GetPropertyExchangeErrorText
# -------------------------------------------------------
sub GetPropertyExchangeErrorText {
    my ($stat) = @_;
    my $ErrorText = "Error executing MT_PropertyExchange.exe.";
    if ($stat eq 0)
        { $ErrorText .= "No error."; }
    elsif ($stat eq 9000)
        { $ErrorText .= "No filename specified or file not found."; }
    elsif ($stat eq 9001)
        { $ErrorText .= "File is read-only."; }
    elsif ($stat eq 9002)
        { $ErrorText .= "Invalid data."; }
    elsif ($stat eq 9003)
        { $ErrorText .= "Application Error."; }
    elsif ($stat eq 9004)
        { $ErrorText .= "File extension unknown."; }
    elsif ($stat eq 9005)
        { $ErrorText .= "Wrong method code. Use \"mode=w\" or \"-mode=r\"."; }
    elsif ($stat eq 9006)
...
\tools\vai_tools\MTPropertyExchange\MT_PropertyExchange.exe

=item *
DLL and documentation files from MT: 
\tools\vai_tools\MTPropertyExchange\build.txt
\tools\vai_tools\MTPropertyExchange\CommandLine.dll
\tools\vai_tools\MTPropertyExchange\dsofile.dll
\tools\vai_tools\MTPropertyExchange\Interop.Dsofile.dll
\tools\vai_tools\MTPropertyExchange\Microsoft.Office.Interop.Excel.dll
\tools\vai_tools\MTPropertyExchange\Microsoft.Office.Interop.Excel.xml
\tools\vai_tools\MTPropertyExchange\Microsoft.Office.Interop.Word.dll
\tools\vai_tools\MTPropertyExchange\Microsoft.Office.Interop.Word.xml
\tools\vai_tools\MTPropertyExchange\Microsoft.Vbe.Interop.dll
\tools\vai_tools\MTPropertyExchange\MT_PropertyExchangeDLL.dll
\tools\vai_tools\MTPropertyExchange\Office.dll
\tools\vai_tools\MTPropertyExchange\Office.xml
